        ...
        public static DataSet GetCustomerName(string order)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(orderType))
                    sQuery = "SELECT OSNOMC, OSTIDO, OSNUOS, OSFECE, OSFENT, OSTIFT, OSFACO FROM [QS36F.RCOSMF00] WHERE OSNUOS = " + orderNo;
                else
                    sQuery = "SELECT OSNOMC, OSTIDO, OSNUOS, OSFECE, OSFENT, OSTIFT, OSFACO FROM [QS36F.RCOSMF00] WHERE OSTIDO IN ('" + orderType + "') AND OSNUOS = " + orderNo;

                //sQuery = "SELECT OSNOMC as nombreCte, OSTIDO OrdenTipo, OSNUOS, OSFECE, OSFENT, OSTIFT as TipoFactura, OSFACO as NoFactura FROM [QS36F.RCOSMF00] WHERE OSTIDO IN ('" + orderType + "') AND OSNUOS = " + orderNo;
